...
window.addEventListener("DOMContentLoaded", function () {
    ...
    function renderPronounPage(pageIndex) {
        ...
        if (!page) {
            if (pronounTitleEl) pronounTitleEl.textContent = "No lesson content";
            if (pronounContentEl) pronounContentEl.innerHTML = "<p>Please check the pronoun lesson data.</p>";
            if (pronounCounterEl) pronounCounterEl.textContent = "0 / 0";
            if (pronounPrevBtn) pronounPrevBtn.disabled = true;
            if (pronounNextBtn) pronounNextBtn.disabled = true;
            return;
        }
        ...
        if (pronounTitleEl) {
            ...
            pronounTitleEl.replaceChildren(danishTitle, englishTitle);
        }
        if (pronounContentEl) {
            pronounContentEl.innerHTML = page.content || "";
            pronounContentEl.classList.remove("grammar-single-column-page");
            markGrammarEnglishTranslations(pronounContentEl);
            arrangeGrammarLanguagePairs(pronounContentEl);
            createGrammarLearningCards(pronounContentEl);
        }
        if (pronounCounterEl) pronounCounterEl.textContent = `${pageIndex + 1} / ${pages.length}`;
        if (pronounPrevBtn) pronounPrevBtn.disabled = pageIndex === 0;
        if (pronounNextBtn) pronounNextBtn.disabled = pageIndex === pages.length - 1;
    }
    ...
    function openPronounLesson(startIndex = 0) {
        ...
        adjectiveLessonEl?.classList.add("is-hidden");
        verbLessonEl?.classList.add("is-hidden");
        pronounLessonEl?.classList.remove("is-hidden");
        currentPronounPage = Math.min(Math.max(startIndex, 0), Math.max(pageCount - 1, 0));
        renderPronounPage(currentPronounPage);
        pronounTitleEl?.focus();
        window.scrollTo({ top: 0, behavior: "smooth" });
    }
    ...
    function renderConjunctionPage(pageIndex) {
        ...
        if (conjunctionTitleEl) {
            ...
            conjunctionTitleEl.replaceChildren(danishTitle, englishTitle);
        }
        if (conjunctionContentEl) {
            conjunctionContentEl.innerHTML = page.content || "";
            conjunctionContentEl.classList.remove("grammar-single-column-page");
            markGrammarEnglishTranslations(conjunctionContentEl);
            arrangeGrammarLanguagePairs(conjunctionContentEl);
            createGrammarLearningCards(conjunctionContentEl);
        }
        if (conjunctionCounterEl) conjunctionCounterEl.textContent = `${pageIndex + 1} / ${pages.length}`;
        if (conjunctionPrevBtn) conjunctionPrevBtn.disabled = pageIndex === 0;
        if (conjunctionNextBtn) conjunctionNextBtn.disabled = pageIndex === pages.length - 1;
    }
    ...
    function openConjunctionLesson(startIndex = 0) {
        ...
        pronounLessonEl?.classList.add("is-hidden");
        verbLessonEl?.classList.add("is-hidden");
        conjunctionLessonEl?.classList.remove("is-hidden");
        currentConjunctionPage = Math.min(Math.max(startIndex, 0), Math.max(pageCount - 1, 0));
        renderConjunctionPage(currentConjunctionPage);
        conjunctionTitleEl?.focus();
        window.scrollTo({ top: 0, behavior: "smooth" });
    }
    ...
    function renderWordOrderPage(pageIndex) {
        ...
        if (!page) {
            if (wordOrderTitleEl) wordOrderTitleEl.textContent = "No lesson content";
            if (wordOrderContentEl) wordOrderContentEl.innerHTML = "<p>Please check the word-order lesson data.</p>";
            if (wordOrderCounterEl) wordOrderCounterEl.textContent = "0 / 0";
            if (wordOrderPrevBtn) wordOrderPrevBtn.disabled = true;
            if (wordOrderNextBtn) wordOrderNextBtn.disabled = true;
            return;
        }
        ...
        if (wordOrderTitleEl) {
            ...
            wordOrderTitleEl.replaceChildren(danishTitle, englishTitle);
        }
        if (wordOrderContentEl) {
            wordOrderContentEl.innerHTML = page.content || "";
            wordOrderContentEl.classList.remove("grammar-single-column-page");
            markGrammarEnglishTranslations(wordOrderContentEl);
            arrangeGrammarLanguagePairs(wordOrderContentEl);
            createGrammarLearningCards(wordOrderContentEl);
        }
        if (wordOrderCounterEl) wordOrderCounterEl.textContent = `${pageIndex + 1} / ${pages.length}`;
        if (wordOrderPrevBtn) wordOrderPrevBtn.disabled = pageIndex === 0;
        if (wordOrderNextBtn) wordOrderNextBtn.disabled = pageIndex === pages.length - 1;
    }
    ...
    function openWordOrderLesson(startIndex = 0) {
        ...
        conjunctionLessonEl?.classList.add("is-hidden");
        prepositionLessonEl?.classList.add("is-hidden");
        wordOrderLessonEl?.classList.remove("is-hidden");
        currentWordOrderPage = Math.min(Math.max(startIndex, 0), Math.max(pageCount - 1, 0));
        renderWordOrderPage(currentWordOrderPage);
        wordOrderTitleEl?.focus();
        window.scrollTo({ top: 0, behavior: "smooth" });
    }
    ...
    openWordOrderLessonBtn?.addEventListener("click", () => openWordOrderLesson(0));
    ...
    pronounToGrammarBtn?.addEventListener("click", openGrammarSelection);
    pronounToHomeBtn?.addEventListener("click", goToMainMenu);
    ...
    wordOrderToGrammarBtn?.addEventListener("click", openGrammarSelection);
    wordOrderToHomeBtn?.addEventListener("click", goToMainMenu);
    ...
    pronounPrevBtn?.addEventListener("click", () => {
        if (currentPronounPage > 0) {
            currentPronounPage--;
            renderPronounPage(currentPronounPage);
            pronounTitleEl?.focus();
            window.scrollTo({ top: 0, behavior: "smooth" });
    ...
    pronounNextBtn?.addEventListener("click", () => {
        const lastPage = (typeof pronomenGrammarPages !== "undefined" ? pronomenGrammarPages.length : 1) - 1;
        if (currentPronounPage < lastPage) {
            currentPronounPage++;
            renderPronounPage(currentPronounPage);
            pronounTitleEl?.focus();
            window.scrollTo({ top: 0, behavior: "smooth" });
    ...
    conjunctionPrevBtn?.addEventListener("click", () => {
        if (currentConjunctionPage > 0) {
            currentConjunctionPage--;
            renderConjunctionPage(currentConjunctionPage);
            conjunctionTitleEl?.focus();
            window.scrollTo({ top: 0, behavior: "smooth" });
        }
    });
    conjunctionNextBtn?.addEventListener("click", () => {
        const lastPage = (typeof konjunktionGrammarPages !== "undefined" ? konjunktionGrammarPages.length : 1) - 1;
        if (currentConjunctionPage < lastPage) {
            currentConjunctionPage++;
            renderConjunctionPage(currentConjunctionPage);
            conjunctionTitleEl?.focus();
            window.scrollTo({ top: 0, behavior: "smooth" });
    ...
    wordOrderPrevBtn?.addEventListener("click", () => {
        if (currentWordOrderPage > 0) {
            currentWordOrderPage--;
            renderWordOrderPage(currentWordOrderPage);
            wordOrderTitleEl?.focus();
            window.scrollTo({ top: 0, behavior: "smooth" });
        }
    });
    wordOrderNextBtn?.addEventListener("click", () => {
        const lastPage = (typeof ordstillingGrammarPages !== "undefined" ? ordstillingGrammarPages.length : 1) - 1;
        if (currentWordOrderPage < lastPage) {
            currentWordOrderPage++;
            renderWordOrderPage(currentWordOrderPage);
            wordOrderTitleEl?.focus();
            window.scrollTo({ top: 0, behavior: "smooth" });
