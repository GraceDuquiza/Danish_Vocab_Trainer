...
window.addEventListener("DOMContentLoaded", function () {
    ...
    function updateDirectionLabel() {
        if (reverseDirectionLabel) {
            reverseDirectionLabel.textContent = reverseModeCheckbox?.checked ? "English → Danish" : "Danish → English";
        }
    }
    ...
    function generateChoices(correctItem, list, answerField) {
        ...
        shuffleArray(others)
            .slice(0, 2)
            .forEach((w) => choices.push(w[answerField]));
    ...
    function startFlashQuiz() {
        ...
        hide(nextQuestionBtn);
        disable(nextQuestionBtn); // until user answers
        hide(prevQuestionBtn);
        disable(prevQuestionBtn); // first question has no prev
    ...
    // ===============================
    //  CATEGORY BUTTONS (enter quiz run)
    // ===============================
